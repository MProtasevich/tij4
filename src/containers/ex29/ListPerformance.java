...
public class ListPerformance {
    ...
    static {
        ...
        tests.add(new Test<List<String>>("set") {
            ...
            public int test(List<String> list, TestParam tp) {
                ...
                int listSize = list.size();
                for(int i = 0; i < loops; i++) {
                    list.set(rand.nextInt(listSize), rg.next());
                }
                return loops;
            }
        });
        ...
        tests.add(new Test<List<String>>("insert") {
            ...
            public int test(List<String> list, TestParam tp) {
                int loops = tp.loops;
                for(int i = 0; i < loops; i++) {
                    list.add(5, rg.next()); // Minimize random-access cost
                }
                return loops;
            }
        });
        tests.add(new Test<List<String>>("remove") {
            ...
            public int test(List<String> list, TestParam tp) {
                ...
                for(int i = 0; i < loops; i++) {
                    list.clear();
                    list.addAll(Arrays.asList(Generated.array(String.class, rg, size)));
                    while(list.size() > 5) {
    ...
    static class ListTester extends Tester<List<String>> {
        ...
        protected List<String> initialize(int size) {
            container.clear();
            container.addAll(Arrays.asList(Generated.array(String.class, rg, size)));
            return container;
        }
    ...
    public static void main(String[] args) {
        ...
        Tester<List<String>> arrayTest = new Tester<List<String>>(null,
            ...
            @Override
            protected List<String> initialize(int size) {
                String[] ia = Generated.array(String.class, rg, size);
